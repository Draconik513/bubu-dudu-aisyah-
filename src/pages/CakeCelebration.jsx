...
const CakeCelebration = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (blowDetectedTimerRef.current)
  ...
  const detectBlow = (stream) => {
    ...
    const analyze = () => {
      ...
      if (volume > 27) {
        if (
          flameRef.current &&
          !flameRef.current.classList.contains("blowing")
        ) {
    ...
    >
      ...
      >
        <h1 className="text-4xl md:text-5xl font-bold text-pink-300 mb-8 neon-text">
          🎉 Happy Birthday{" "}
          <span className="text-white">Aisyah Nur Anjani</span>! 🎉
        </h1>
        ...
        >
          Hore! Lilinnya padam! Selamat Ulang Tahun Aiss!
        </motion.div>
      ...
      <style jsx global>{`
        ...
        @keyframes neon {
          from {
            text-shadow: 0 0 5px #fff, 0 0 10px #fff, 0 0 15px #f43f5e,
              0 0 20px #f43f5e;
          }
          to {
            text-shadow: 0 0 10px #fff, 0 0 20px #fff, 0 0 30px #f43f5e,
              0 0 40px #f43f5e;
          }
        }
      `}</style>
